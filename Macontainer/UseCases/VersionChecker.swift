...
struct VersionChecker {
    ...
    private static func fetchLatestVersion() async throws -> String {
        guard let url = URL(string: "https://api.github.com/repos/apple/container/releases/latest") else {
    ...
    private static func parseVersion(from versionString: String) -> String {
        ...
        if let regex = try? NSRegularExpression(pattern: pattern, options: []),
           let match = regex.firstMatch(in: versionString, options: [], range: NSRange(location: 0, length: versionString.utf16.count)),
